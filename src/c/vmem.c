...
void setup_spt(pte_t pt[5][PTECOUNT], unsigned long paddr_base)
{
  ...
  pt[2][0] = (SPA2GPA_C((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_U | PTE_D | PTE_A;
  ...
  pt[4][PTECOUNT-1] = (SPA2GPA_C((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_D | PTE_A;
}
 ...
 *
 * @details Sets up a page table where:
 * - `0x000_0000_0000_0xxx` is mapped to paddr_base
 * - `0x000_0000_002x_xxxx` is mapped to paddr_base superpage
 *
 * @param pt Page table base address
...
void setup_gpt(pte_t pt[3][PTECOUNT], unsigned long paddr_base)
{
  ...
  pt[1][1] = ((pte_t)paddr_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_U | PTE_D | PTE_A;
