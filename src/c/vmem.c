 ...
 * - 0x000_0000_0000_0xxx is mapped to base_addr as user page
 * - 0xfff_ffff_ffff_fxxx is mapped to base_addr as supervisor page
 * @param pt_base Page table base address
 */
void setup_pt(pte_t pt[PAGECOUNT][PTECOUNT])
{
  pt[0][0] = ((pte_t)pt[1] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
  pt[1][0] = ((pte_t)pt[2] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
  pt[2][0] = ((pte_t)DRAM_BASE >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_U | PTE_D | PTE_A;

  pt[0][PTECOUNT-1] = ((pte_t)pt[3] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
  pt[3][PTECOUNT-1] = ((pte_t)pt[4] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V;
  pt[4][PTECOUNT-1] = ((pte_t)DRAM_BASE >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_D | PTE_A;
}
